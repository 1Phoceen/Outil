...
const LANGUAGE_CONFIG = {
  en: {
    name: "anglais",
    expectedCols: 4,
    formatHint: "francais,base verbale,preterit,participe passe",
    placeholder: "aller,go,went,gone\nvoir,see,saw,seen",
    promptPrefix: "Complete les autres formes anglaises a partir de :",
    randomGivenKeys: ["fr", "base", "preterit", "participle"],
    forms: [
      { key: "fr", label: "Francais" },
      { key: "base", label: "Base verbale" },
      { key: "preterit", label: "Preterit" },
      { key: "participle", label: "Participe passe" }
    ]
  },
  de: {
    name: "allemand",
    expectedCols: 5,
    formatHint: "francais,infinitif,preterit,participe passe,parfait",
    placeholder: "aller,gehen,ging,gegangen,ist gegangen\nvoir,sehen,sah,gesehen,hat gesehen",
    promptPrefix: "Complete les autres formes allemandes a partir de :",
    randomGivenKeys: ["fr", "base", "preterit", "participle", "perfect"],
    forms: [
      { key: "fr", label: "Francais" },
...
function getVerbDisplay(verb) {
  const values = [verb.base, verb.preterit, verb.participle];
...
function loadListFromStorage() {
  ...
  try {
    ...
    if (saved && typeof saved === "object" && saved.byLanguage) {
      storageState.currentLanguage = LANGUAGE_CONFIG[saved.currentLanguage] ? saved.currentLanguage : "en";
      ...
      const legacyLanguage = LANGUAGE_CONFIG[saved.language] ? saved.language : "en";
